...
\[
    \begin{cases}
        ボールの最高点の位置をy=h（図\ref{butsuri}参照） & \\
        ボールを蹴り上げた時点でのボールの位置をy=-h（図\ref{butsuri}，図\ref{enzan}参照）と仮定する &
    \end{cases}
\]
...
入った場合，ユーザの骨格座標の再追従が上手く行われない場合があった．
その問題を解決する手段として，カスケード分類器を用いた手法を提案する．
...
\clearpage
\subsubsection{手法}
節\ref{kokkaku}同様ユーザにCombination画面の投影を行う．

サッカーモードにおいて，ユーザの動作をカスケード分類器を用いて検出する．

検出した動作に応じて，スケルトン座標を用いらずに
インタラクティブにプロジェクションマッピングが変化する．


cascade画面は黒い背景画像にGray画面を合成したもの．図
その画面に作成したcascade分類器を用いてユーザの検出を行う．
